 ...
 * performance for put, get and remove.
 *
...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
	 ...
	 * 
	 * @param compare is comparator is be used
	 */
    ...
    private void rebalance(Position<Entry<K, V>> p) {
    	//Track the hold height of position p
        int oldHeight = 0;
        int newHeight = 0;
        ...
        do {
        	...
        	if (!isBalanced(x)) {
        		//Find children with taller height
        		Position<Entry<K, V>> child = tallerChild(x);
        		//Find grandchildren with taller height
