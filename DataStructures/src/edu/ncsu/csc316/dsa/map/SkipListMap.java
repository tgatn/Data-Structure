...
public class SkipListMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	public V put(K key, V value) {
		...
		if (!isSentinel(temp) && compare(temp.getEntry().getKey(), key) == 0) {
			...
			while (temp != null) {
				MapEntry<K, V> setVal = (MapEntry<K, V>) temp.getEntry();
		...
		do {
			...
			q = insertAfterAbove(temp, q, entry);
	...
	/**
	 * Writes a string representation of the SkipListMap
	 * @return the string representation of the SkipListMap
	 */
