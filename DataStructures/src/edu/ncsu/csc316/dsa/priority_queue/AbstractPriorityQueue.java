...
public abstract class AbstractPriorityQueue<K extends Comparable<K>, V> implements PriorityQueue<K, V> {
	...
	/** comparator used to determine how to sort a list of item */
	...
	/**
	 * Delegates to the comparable's compareTo implementation that defines the
	...
	public class NaturalOrder implements Comparator<K> {
		/**
		 * Compares the ordering of the first and second parameters
		 * 
		 * @param first  is first value to compare
		 * @param second is second value to compare
		 * @return an integer -1, 0, or 1 which represents if first is less than, equal
		 *         to, or greater than the second parameter
		 */
	 ...
	 * @return a new entry with a key and value from the parameters
	 */
